...
const WinModal = ({ time, moves, onRestart, onNewGame, dark }) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-6">
      <div
        className={`w-full max-w-[654px] rounded-2xl p-8 md:p-14 flex flex-col items-center transition-colors duration-300 ${
          dark ? "bg-navy-light" : "bg-white"
        }`}
      >
        <h2
          className={`text-2xl md:text-5xl font-bold mb-4 ${dark ? "text-white" : "text-navy"}`}
        >
          You did it!
        </h2>
        ...
        <div className="w-full space-y-4 md:space-y-6 mb-8 md:mb-14">
          <div
            className={`flex justify-between items-center px-6 md:px-8 py-4 md:py-6 rounded-xl ${
              dark ? "bg-navy" : "bg-gray-light"
            }`}
          >
            <span className="text-gray-text font-bold md:text-xl">
              Time Elapsed
            </span>
            <span
              className={`text-xl md:text-3xl font-bold ${dark ? "text-white" : "text-navy"}`}
            >
              {time}
            </span>
          </div>

  ...
  return (
    <div
      ...
      className={`perspective-1000 cursor-pointer ${
        is6x6 ? 'w-10 h-10 md:w-20 md:h-20 text-lg md:text-4xl' : 'w-16 h-16 md:w-28 md:h-28 text-2xl md:text-5xl'
    ...
    >
      ...
      >
        {/* Card Back */}
        <div className={`absolute inset-0 rounded-full backface-hidden transition-colors ${
          dark ? 'bg-navy hover:bg-navy-light' : 'bg-navy-light hover:bg-gray-hover'
        }`} />
        ...
        >
          {theme === 'Icons' ? (
            <div className={`${is6x6 ? 'scale-75 md:scale-100' : 'scale-110 md:scale-150'}`}>
              {value}
            </div>
          ) : (
            value
